class Top_Company_Cost_Count:
    ...
    def csv_options(self):
        ...
        self.data = self.data[
            self.data["document_type"].str.contains("ADDITIONAL") == False]  # удаляем строки, которые не подходят по условию
        self.data = self.data[((self.data["status"].str.contains("COMPLETED")) | (self.data["status"].str.contains("COMPLETED_SIGNED"))) == True ]  # удаляем строки, которые не подходят по условию
        ...
        self.data = self.data.loc[
            (self.data["general_date_transaction"] >= self.time_start) & (self.data["general_date_transaction"] <= self.time_end)]
        self.data = self.data.loc[self.data['roster_item_code'] == self.code]
